...
public:
  dimensioned(dimensions_type dimensions) : dimensions(dimensions) {}

  const dimensions_type &dims() const noexcept { return this->dimensions; }
};
...
private:
  ...
  private:
    ...
    iterator_internal(decltype(line) line) : line(line) {}
  ...
  public:
    ...
    value_type operator*() noexcept { return this->line; }

    const_value_type operator*() const noexcept { return this->line; }

    const value_type *operator->() noexcept { return &this->line; }

    const const_value_type *operator->() const noexcept { return &this->line; }

    iterator_internal &operator++() noexcept {
      this->line = utki::make_span(this->line.data() + this->line.size(),
                                   this->line.size());
      return *this;
    }
    ...
    iterator_internal &operator+=(difference_type d) noexcept {
      this->line = utki::make_span(this->line.data() + d * this->line.size(),
                                   this->line.size());
    ...
    friend iterator_internal operator+(difference_type d,
                                       const iterator_internal &i) noexcept {
      return i + d;
    }
    ...
    value_type operator[](difference_type d) noexcept { return *(*this + d); }
...
public:
      ...
      : dimensioned(dimensions),
        buffer(this->dimensions.x() * this->dimensions.y()){
            ASSERT(!this->buffer.empty() ||
                   (this->dimensions.x() == 0 && !this->buffer.data()))}
  ...
  iterator end() noexcept {
    return iterator(utki::make_span(
        this->buffer.data() + this->dimensions.x() * this->dimensions.y(), 0));
  }

  const_iterator cbegin() const noexcept {
    return const_iterator(
        utki::make_span(this->buffer.data(), this->dimensions.x()));
  }
  ...
  utki::span<pixel_type> pixels() noexcept { return this->buffer; }

  utki::span<const pixel_type> pixels() const noexcept { return this->buffer; }
